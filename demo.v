...
Theorem correct:
  forall {n : nat} (SLE : TriDiagSys n), is_consistent SLE -> matrix_mul SLE (solution SLE) = f _ SLE.
Proof.
intros. 
induction n.
  - (*База индукции n = 0*)
    unfold is_consistent, solution, matrix_mul.
    simpl.
    destruct SLE as [c b a f].
    unfold div, sub, mul, add, denominator, alpha_i, beta_i, x_j, x_n, alpha_1, beta_1.
    simpl.
    rewrite <- no_brackets with (r1 := hd b) (r2 := hd f) (r3 := hd b).
    unfold Rdiv.
    rewrite Rinv_r_simpl_m with (r1 := hd b) (r2 := hd f).
    +
     unfold hd.
